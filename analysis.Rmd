...
fitRF<-train(classe~.,data=trainingSet,model="rf",trControl=control)
```

## Cross-validating the model

To have an out-of-sample cross-validation of the model, we build a confusion matrix for the `testingSet` (that we haven't used in the `train` function)
...
Hence, the out-of-sample accurary is `0.9969` :-)
